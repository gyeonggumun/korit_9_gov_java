...
public class UserRepository {
    private static UserRepository instance; // 데이터를 보호하기 위하여 변수명 앞에 private사용
    private User[] users;
    private int autoIncrementId;

    private UserRepository() { // 외부에서 함부로 생성할 수 없도록 하기위해 생성자에 private 사용
        users = new User[0];
        autoIncrementId = 1;
    }

    public static UserRepository getInstance() {  // instance함수가 계속 호출될 때 생성이 계속 일어나지 않도록 조건사용
